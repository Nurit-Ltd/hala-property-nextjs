...
const MapViewPage = () => {
  ...
  const tabs = [
    { id: 1, label: "In Demand", name: "InDemand", icon: <DiamondIcon /> },
    { id: 2, label: "ROI", name: "ROI", icon: <RoIcon /> },
    { id: 3, label: "Project Rating", name: "ProjectRating", icon: <RatingIcon /> },
    { id: 4, label: "New", name: "New", icon: <NewIcon /> },
    { id: 5, label: "Rental Yield", name: "RentalYield", icon: <RentalIcon /> },
  ];

  const handleTablClick = (id) => {
    setActiveTab(id);
    // setActiveCategory
    setActiveCategory(tabs.find((tab) => tab.id === id).name);
  };
  const Center = {
    // 25.190583, 55.362625
    lat: 25.180447,
    ...
    <>
      <div className="    overflow-hidden w-full relative">
          ...
          <div>
            ...
            <div className="lg:absolute top-6 left-24 ">
              <div className="flex items-center gap-4 rounded-[42px] p-3 bg-white">
                {tabs.map((tab) => (
                  <Button key={tab.id} onClick={() => handleTablClick(tab.id)} variant="outline" className={cn("rounded-full  bg-[#F6F5F1] text-[#323B49] border-grey400 py-2.5 px-4 gap-2", activeTab === tab.id && "bg-primary/5 text-primary border border-primary")}>
                    {tab.icon} {tab.label}
                  </Button>
                ))}
              </div>
          ...
          <div className={`${isMapView ? "hidden" : "block"}`}>
            ...
            <div className="lg:absolute top-6 left-24 overflow-hidden">
              <div className="flex items-center gap-4 rounded-[42px] p-3 bg-white overflow-y-scroll custom-scrollbar-tab">
                {tabs.map((tab) => (
                  <Button key={tab.id} onClick={() => setActiveTab(tab.id)} variant="outline" className={cn("rounded-full  bg-[#F6F5F1] text-[#323B49] border-grey400 py-2.5 px-4 gap-2", activeTab === tab.id && "bg-primary/5 text-primary border border-primary")}>
                    {tab.icon} {tab.label}
                  </Button>
                ))}
              </div>
        ...
        <div className=" lg:hidden fixed bottom-0 z-50 px-4 w-full bg-white h-[78px] flex items-center justify-center">
          {isMapView ? (
            <Button onClick={() => setIsMapView(!isMapView)} className="bg-primary w-full">

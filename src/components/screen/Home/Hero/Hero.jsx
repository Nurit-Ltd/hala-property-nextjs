...
const Hero = () => {
  ...
  return (
    <section className=" bg-[url('/hero-banner.png')] w-full h-[292px] lg:h-[400px] 2xl:h-[700px] bg-no-repeat bg-cover flex items-center justify-center">
      <div className="container_fluid">
        <div className="flex flex-col items-center justify-center space-y-4 lg:space-y-8">
          <div className="max-w-[903px] mx-auto space-y-2 lg:space-y-3">
            <h1 className="text-2xl lg:text-[40px] lg:leading-[60px] 2xl:text-[54px] 2xl:leading-[93px] font-Merriweather font-bold text-white text-center">The Future of Real Estate</h1>
            <h3 className="max-w-max sm:max-w-[520px] lg:max-w-[777px] mx-auto text-sm leading-[18px] lg:text-xl lg:leading-[150%] text-center text-white">Experience a smarter, faster way to find your perfect property. Discover luxurious homes and investment opportunities tailored to your lifestyle, all at your fingertips.</h3>
          </div>
                    ...
                    <SelectTrigger className="w-full lg:min-w-[140px] text-sm border-none bg-transparent ring-0 focus:ring-0 outline-none shadow-none flex items-center gap-2 text-grey600">
                      ...
                      <div className="flex flex-col items-start">
                        ...
                        {item.title === "Bedrooms" ? (
                          ...
                          <span>
                            AED {formatToMillion(priceRange[0])} - AED {formatToMillion(priceRange[1])}
                          </span>
                        ) : (
                          <>
                            <span className="hidden md:inline-block">Set your budget</span>
                            <span className="inline-block md:hidden">Budget</span>
                          </>
                        )}
                      </div>
                    </SelectTrigger>
                      ...
                      {item.type === "buttonGroup" && (
                        <div>
                          <div className="flex items-center justify-between">
                            ...
                            <button className="text-primary text-sm" onClick={() => setSelectedBedrooms("Select bedrooms")}>
                              Reset
                            </button>
                          </div>
                          <div className="flex mt-4 border border-[#CBD5E0] rounded-[6px] overflow-hidden">
                            {item.options.map((option) => (
                              <button key={option} onClick={() => handleBedroomSelect(option)} className={`px-4 lg:px-6 py-1.5 border-r border-[#CBD5E0] last:border-none ${selectedBedrooms === option ? "bg-[#CBD5E0] text-primary" : "text-gray-700"}`}>
                                {option}
                              </button>
                            ))}
                          </div>
                      ...
                      {item.type === "rangeSlider" && (
                        <div className="mb-1">
                          ...
                          <div className="flex justify-between gap-2 text-sm text-gray-500 mt-4">
                            <span className="bg-[#F1F2F4] border border-grey400 py-0.5 px-2 w-full rounded-[4px] text-xs text-grey700">{priceRange ? `AED ${formatToMillion(priceRange[0])}` : "Set minimum"}</span>
                            <span className="bg-[#F1F2F4] border border-grey400 py-1 px-2 w-full rounded-[4px] text-xs text-grey700">{priceRange ? `AED ${formatToMillion(priceRange[1])}` : "Set maximum"}</span>
                          </div>
                          <RangeSlider min={500000} max={8500000} step={100000} value={priceRange || [500000, 8500000]} onInput={handlePriceChange} className="range-slider w-full mt-4" />
                        </div>
                      )}
                    </SelectContent>
                  </Select>

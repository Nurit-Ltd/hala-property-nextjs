          ...
          <div className="w-full h-12 flex items-center justify-between">
            ...
            <ul className="flex items-center gap-1.5">
              ...
              <div ref={dropdownRef} className="relative inline-block text-left">
                <button onClick={toggleDropdown} className={`inline-flex items-center gap-1.5 px-3 py-1.5 ${isOpen ? "text-white border-b-[1.5px] border-white font-bold duration-200" : "text-grey500 font-semibold"}`}>
                  More <IoIosArrowDown />
                </button>
                ...
                <div className={`absolute right-0 mt-2 w-[138px] bg-white border border-grey300 p-2 rounded-lg transition-all duration-300 transform ${isOpen ? "opacity-100 scale-100 visible" : "opacity-0 scale-95 invisible"}`}>
                  <div className="space-y-2">
                    <Link href="/about-us" passHref className="block px-3 py-1.5 font-medium text-grey600 hover:bg-primary50 hover:text-grey900 duration-300 rounded">
                      About Us
                    </Link>
                    <Link href="/contact-us" passHref className="block px-4 py-2 font-medium text-grey600 hover:bg-primary50 hover:text-grey900 duration-300 rounded">
                      Contact Us
                    </Link>
                    <Link href="/blog" passHref className="block px-4 py-2 font-medium text-grey600 hover:bg-primary50 hover:text-grey900 duration-300 rounded">
                      Blog
                    </Link>
                  </div>
                </div>
            ...
            <div className="flex items-center gap-6">
              <Link href={"#"} className="w-10 h-10 rounded-full bg-white/[10%] backdrop-blur-[9px] flex items-center justify-center relative">
              ...
              <div className="flex items-center gap-2 xl:gap-4">
                <Link href={"#"} className="px-2 xl:px-5 py-2 h-12 rounded-lg border border-white/[30%] nav-ai-button-box flex items-center justify-center gap-1 xl:gap-2 text-xs xl:text-base text-white font-medium">
                  <Image src={starIcon} alt="starIcon" />
                  Find Property with AI
                </Link>
                <Link href={"/auth"} className="h-12 px-3 xl:px-8 py-2 bg-gradient-to-r from-[#0E5DF4] to-[#004BDC] border border-black/[8%] text-xs xl:text-base rounded-lg flex items-center justify-center text-white font-semibold ">
                  Sign in
                </Link>
              </div>
            </div>
